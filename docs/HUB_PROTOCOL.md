...
### `POST /team-task/verify` — verify a hub team task from external evidence
...
A hub client's `team_task complete` moves its canonical task to `VERIFYING`; it may then
...
`cline-team:<encoded-workspace>:<taskId>` (the Cline-surface task namespace) so
task IDs cannot collide across workspaces in the shared graph. A bridge without
a workspace root uses the legacy-local `cline-team:<taskId>` form.
...
discovery, and the ordinary hub token exposed to hub clients is rejected. The daemon
also provisions the verifier capability in `hub/verifier.json` (mode `0600`),
separate from the ordinary `hub/discovery.json`, for trusted local verification
