...
`test/pretrust-parsing-audit.test.ts` uses a fixture workspace containing poisoned project-local config and instruction files. It verifies that inventory-covered startup helpers resolve paths and generate hub-owned configuration without opening or parsing those fixture files, while explicit post-trust skills discovery remains the only path that reads a skill file.
...
- Ordering regression tests: added in `test/pretrust-parsing-audit.test.ts`.
