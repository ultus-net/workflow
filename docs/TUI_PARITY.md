...
| Usage/cost readout (composer footer) | **Parity + per-turn** — live cumulative `tokens · $cost` plus the last turn's delta (`+P this turn · $c`) in the header status line (refreshed once per second) from the metering proxy; the per-turn figure is computed at turn boundaries and failed/cancelled turns publish nothing (the web item is composer-footer; the terminal renders it in the header status box; web shows cumulative + context size) | `test/tui-tasklist.test.ts` (usage meter), `test/usage-tracker.test.ts` (turn boundaries) |
