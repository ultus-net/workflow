...
| goose (AAIF, ambient) | unknown — subagents are disabled in manual-approval modes (`docs/GOOSE_RESEARCH.md` §5), so under `GOOSE_MODE=approve` the SUBAGENT probe expects the delegate tool ABSENT or denied | pending the probes | pending the probes | **No verdict — wired and gated, no live runs yet** (the skills-delivery-probe precedent). W048 landed the third `WORKFLOW_ACP_AGENT` kind: a contained `goose acp` launch profile (`src/integrations/goose-agent-config.ts` — approve mode, telemetry off, per-runtime `GOOSE_PATH_ROOT` config, OpenRouter through the loopback metering proxy with a placeholder-only credential inside the boundary, azure_foundry direct with hub-injected key env, unit-tested) and the six gated probes from the qualification plan (`test/acp-goose-{permission,subagent,mcp-mount,resume,metered,hooks}-probe.test.ts`). Enforcement classification is earned ONLY by live probe evidence — goose carries no `enforced` claim and stays outside spawn-granting policy until the probes run. Run with the gate envs (`WORKFLOW_ACP_GOOSE_PERMISSION=1` etc. plus provider credentials; see each probe's header) and record per-probe verdicts here — the weekly goose release cadence makes it the first pinned-version-sensitive agent: record the ambient version in `agentInfo` evidence and re-run the family on every bump. |
