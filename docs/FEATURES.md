...
| OpenCode conformance probes (subagent, MCP mount, resume, skills delivery) | Complete | `test/acp-opencode-subagent-probe.test.ts`, `test/acp-opencode-mcp-mount-probe.test.ts`, `test/acp-opencode-resume-probe.test.ts`, `test/acp-opencode-skills-delivery-probe.test.ts` (env-gated; `--pure` launches measure the stock surface without operator plugins) ran live 2026-09-16 on 1.18.31 — **spawn Green (the `task` tool call projected and its permission reached the hub, so OpenCode is `enforced`-eligible per `docs/HOST_ADAPTERS.md` when the hub pins the ask config); hub-written MCP config mounts on both surfaces (project `opencode.json` and `XDG_CONFIG_HOME` — F1/G3 unblocked on OpenCode); `session/load` restores model context across restart (exact keyword recalled); the skills-mcp mount delivers the operator's skill catalog (`WORKFLOW_ACP_OPENCODE_SKILLS`)**; re-run whenever the ambient `opencode` version changes (recorded in each probe's `agentInfo` evidence) |
