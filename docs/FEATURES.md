...
| Hub-owned reviewer auto-launch on run completion | Complete | `createRunRegistry` reviewer seam (`src/integrations/run-registry.ts`) + production wiring (`src/integrations/hub-run-gates.ts`, `src/cli/hub.ts`) — contained ACP reviewer, contained git-diff sourcing, fail-closed verdict parsing; auto-launch fired live 2026-09-16 via the scheduled-turn probe chain and **closed fail-closed: the reviewer's git-diff sourcing shell has no active task to authorize against (pre-existing composition defect the probe exposed), so the run stays VERIFYING with a surfaced blocking reason — never a silent pass; fix tracked in the review follow-up ledger** |
| Hub-run test evidence (`test:<workspace>`) | Complete | requirement declared only when a real `WORKFLOW_TEAM_TASK_VERIFY_COMMAND` is configured (never the `true` default); `begin()` stales the workspace test subject so runs cannot inherit a predecessor's green tests; **not yet exercised live end-to-end: the verify flow runs the reviewer gate first, and with the reviewer closing fail-closed (see the reviewer row) the test command has not yet executed in a live chain** — gate behavior is unit-tested (`test/hub-runs.test.ts`); reaching it live requires the reviewer-shell authorization fix |
