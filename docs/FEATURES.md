...
| Hub-run test evidence (`test:<workspace>`) | Complete | requirement declared only when a real `WORKFLOW_TEAM_TASK_VERIFY_COMMAND` is configured (never the `true` default); `begin()` stales the workspace test subject so runs cannot inherit a predecessor's green tests; **not yet executed in a live chain: the verify flow satisfies on fresh reviewer approval before the test gate is reached** (its engagement is unit-tested in `test/hub-runs.test.ts`); reaching it live requires either a reviewer rejection followed by a passing test run, or a composition change |
