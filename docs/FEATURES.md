...
| Review provenance and replay (W041) | Complete | `src/review/provenance.ts` + `src/integrations/review-provenance-store.ts` + runner journaling in `src/integrations/hub-reviewer.ts` — every hub review decision is journaled (NDJSON, `~/.workflow/review-provenance.jsonl`, `WORKFLOW_HUB_PROVENANCE` overrides, fail-closed corrupt-journal loads, bounded trims) bound to its fingerprint: commit hash, prompt digest, W039 manifest digest, W040 partition digest, and a rule-set digest over every rule table; approvals journal BEFORE recording (unprovenanced approvals never record); interrupted partitioned reviews resume only units whose NEWEST same-fingerprint record was an approval with complete coverage — newer rejections shield older approvals, any fingerprint change resumes nothing, foreign workspaces never replay here; full-replay approval (zero fresh sessions) is possible by design and always journaled as `resumed from provenance: N`; the journal is 0600 same-user hub state disciplining honest clients like the verifier token — mutations DURING a review remain the kernel epoch's job; direct `/run/review` submissions stay unjournaled; tests: `test/review-provenance.test.ts`, `test/review-provenance-store.test.ts`, `test/hub-reviewer.test.ts` |
