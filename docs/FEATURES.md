...
| Executable security assurance case (W042) | Complete | `docs/SECURITY_ASSURANCE.md` + `test/security-assurance.test.ts` — twelve surface sections binding every material security claim to its implementation boundary plus a machine-parseable citation (`test/<file>#"<exact title>"`, `gated[ENV]: ...` for live probes, `manual[npm run ...]` for host-gated commands); the checker parses all 130+ citations and fails when any cited test, title, gate, or script ceases to exist, requires a verification on every claim row, guards against gutting (section/count floors), and pins the honesty statements; 19 known residual risks stay stated (C3 model-channel exfiltration, G7 compaction, same-user honest-client limits, transport residuals); `THREAT_MODEL.md` cross-references the map |
