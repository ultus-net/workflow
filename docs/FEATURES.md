...
| Hub-run test evidence (`test:<workspace>`) | Complete | requirement declared only when a real `WORKFLOW_TEAM_TASK_VERIFY_COMMAND` is configured (never the `true` default); `begin()` stales the workspace test subject so runs cannot inherit a predecessor's green tests; test-evidence gate verified live 2026-09-16 via the scheduled-turn probe pass (real verify command, run reached VERIFIED) |
