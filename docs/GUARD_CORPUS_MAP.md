...
| Symlink path bypasses (ancestor resolution, dangling symlinks, symlinked worktree exclusions) | Hub workspace confinement (`pathWithinWorkspace`, canonical-existing-path) + `workflow-fs-exec-mcp` `resolveWithinWorkspace` with symlink-ancestor realpath (`test/hub-client.test.ts` fs-bounds) | **Ported** |
...
G1 opencode ask-config probe; C1 scheduled real-agent turn; the
`workflow-fs-exec-mcp` mount probe (Cline ACP MCP-config surface);
`acp-cline-tool-matrix` re-run per pinned Cline bump.