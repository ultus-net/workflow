...
| Review-scope coverage (whole-diff review incl. untracked files) | W039 manifest: `src/review/manifest.ts` (deterministic scope + obligations + digest) + hub-reviewer `[COVERAGE]` approval gate (`test/review-manifest.test.ts`, `test/hub-reviewer.test.ts`); binds reviewer claims to the scope list — process discipline like the ≥3-axis rule, not proof of reading. Extended by W040: multi-component manifests are partitioned into per-unit reviewers (focused rules, ≤12-file units, an integration review exactly when >1 unit, every unit must approve with complete `[COVERAGE]`) and by W041: every decision is journaled to hub-state provenance (commit/prompt/diff/manifest/partition/rule-set fingerprint) before it is recorded, and interrupted partitioned reviews resume only exact-fingerprint units | **Ported** |
...
probe file's own commentary and in the fail-closed cases of
`test/hub-reviewer.test.ts` (with the prompt-contract lines also recorded in
`docs/ACP_DECISION.md`));
