...
| The hub fails closed on an invalid workspace declaration | `src/integrations/run-registry.ts:59` | test/hub-workspace.test.ts#"hub fails closed on an invalid workspace declaration" |
| Workspace aliases canonicalize to one real path before task identity | `src/integrations/run-registry.ts:64` | test/hub-workspace.test.ts#"canonicalWorkspace canonicalizes aliases to one real path" |
...
| goose ACP | permission probe | gated[WORKFLOW_ACP_GOOSE_PERMISSION]: test/acp-goose-permission-probe.test.ts#"goose PERMISSION probe: approve-mode interception with usable reject options and honored denials" | Green (live 2026-09-17, 1.50.1): every mutating call asked with full reject options, denials honored |
| goose ACP | subagent probe | gated[WORKFLOW_ACP_GOOSE_SUBAGENT]: test/acp-goose-subagent-probe.test.ts#"goose SUBAGENT probe: no unprojected spawns under approve mode; delegate absent or denied" | Green (denied): `delegate` projected and was denied at the hub — spawn-classified on this evidence |
| goose ACP | MCP mount | gated[WORKFLOW_ACP_GOOSE_MCP_MOUNT]: test/acp-goose-mcp-mount-probe.test.ts#"goose MOUNT probe: hub-written GOOSE_PATH_ROOT config mounts the skills-mcp stdio extension" | Green: the documented map-shaped config.yaml mounts skills-mcp; list_skills verbatim |
| goose ACP | resume probe | gated[WORKFLOW_ACP_GOOSE_RESUME]: test/acp-goose-resume-probe.test.ts#"goose RESUME probe: session/load restores transcript and model context across a contained restart" | Green: transcript replayed, exact keyword recalled across a contained restart |
| goose ACP | metered probe | gated[WORKFLOW_ACP_GOOSE_METERED]: test/acp-goose-metered-probe.test.ts#"goose METERED probe: placeholder-only credential inside the boundary; proxy records traffic; usage channels confirmed" | Green (openrouter): placeholder-only credential, proxy records traffic, two usage_update channels project; azure_foundry pending credentials |
| goose ACP | hooks probe | gated[WORKFLOW_ACP_GOOSE_HOOKS]: test/acp-goose-hooks-probe.test.ts#"goose HOOKS probe: PreToolUse deny works under containment and blocks the mutation" | Green: documented-structure deny plugin with on_failure block stopped the mutation under containment |
| goose ACP | subagent hooks probe | gated[WORKFLOW_ACP_GOOSE_SUBAGENT_HOOKS]: test/acp-goose-subagent-hooks-probe.test.ts#"goose SUBAGENT-HOOKS probe: granted spawn; PreToolUse coverage of subagent-internal tool calls classified" | NEGATIVE (live 2026-09-17, twice, 1.50.1): the delegated subagent's file-write fired no PreToolUse record and projected no ACP tool_call update — only top-level calls and the delegate spawn itself intercept; evidence-bounded input to the W050 SDK-seam decision |
...
15. **G7 compaction is outside hub control over ACP** — the plugin-era compaction-time gating has no ACP equivalent; documented regression, revisited when the ACP compaction RFD stabilizes. Mitigation (W047): restart discipline, resume-backed (session-id projection + `WORKFLOW_ACP_RESUME` + probe-proven `session/load` context restoration); context/usage signals now project into the session record as advisory `agent-context` events — visibility, never upgraded to control.
