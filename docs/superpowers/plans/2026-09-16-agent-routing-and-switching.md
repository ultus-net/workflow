...
- **Webhook gateways terminate at the hub.** goose ships a pluggable inbound gateway surface — `goose gateway start <platform>` with secure pairing and persistent auto-compacting sessions (Telegram first, aaif-goose/goose PR #7199) — and its integration gateways are webhook-based. In hub-composed deployments the webhook receiver belongs to the hub: inbound messages terminate at a hub-owned boundary, pair and authorize there, and dispatch to a runtime like any other prompt. Nothing external talks to a contained agent directly; a webhook payload is untrusted input. Recorded as a THREAT_MODEL follow-up. The same boundary would serve a future assistant-facing entry point (assistant conversations from Telegram/mobile arriving through the hub and executing on the goose runtime — the role-routing rule above applied to inbound traffic).
