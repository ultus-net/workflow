...
1. **Pool definition:** default alias pool = `deepseek-flash` (V4.1-Flash), `glm-5.3`, `glm-5.3-flash`, `kimi-k3`. Endpoint strategy: **direct vendor endpoints first** (all three documented above; cost + caching benefits), **OpenRouter as uniform fallback** for any model/endpoint that is unavailable — the resolver must verify IDs live (API or vendor docs) and record evidence before committing names; no guessed IDs.
   ...
   > **Correction (2026-09-19, operator):** the pivot's operative mechanism is the **OpenRouter Auto Router alias pool**, not this stack-level composition. The agent default stays `openrouter/auto`; the operator-configured `~…-latest` open-source alias set (`WORKFLOW_OPENROUTER_AUTO_ALIASES`, e.g. `~deepseek/deepseek-flash-latest`, `~z-ai/glm-latest`, `~z-ai/glm-flash-latest`, `~moonshotai/kimi-latest`) is resolved against the live catalog and injected as the Auto Router's `allowed_models` — the routing itself happens at OpenRouter. The per-vendor direct-endpoint composition below is the **opt-in cost path**: it engages only when a vendor key is configured (direct cost + context-caching benefits); families without keys route through OpenRouter. This supersedes the "direct vendor endpoints first" emphasis of this slice.
