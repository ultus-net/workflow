...
test("emits MCP log and progress notifications for tool calls", async () => {
  ...
  await client.callTool({ name: "find_relevant_tests", arguments: { workspaceRoot: process.cwd(), file: "test/discovery.test.ts", limit: 1 }, _meta: { progressToken: "rollout-probe" } });
