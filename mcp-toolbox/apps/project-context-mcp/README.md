...
Repository text is returned explicitly as `untrusted_repository_content`. The server does not interpret instructions, select a next task, mutate repository files, or mutate a host's task list. A source is read only after its opened file identity matches the canonical path confined to the workspace, so source replacement cannot transfer a validated read outside that boundary. Snippets are capped at 4096 bytes, discovery returns at most 20 candidates, and oversized planning directories fail closed.
