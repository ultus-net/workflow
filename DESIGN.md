...
- **One settings surface.** Every operator preference lives in the settings dialog (`settings-dialog.tsx`, opened by the composer gear or Ctrl/Cmd+,): appearance and focus mode, all agent-advertised options (model, effort, mode, tool toggles — synced with the composer-adjacent pickers through the same state), approvals and capabilities, transcript and notifications, the keyboard map, and the session's authority facts. Composer-adjacent pickers remain for speed; the dialog is the complete map. It is a sectioned list of rows, not a card grid; `role="dialog"` with focus trap, Esc, and focus return.
