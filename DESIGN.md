...
- **Pages behind top-bar nav slugs.** A slim header carries the focus-mode toggle, the `Workflow` wordmark, **nav slugs (icon over name, square, mono)** for **Chat · Agents · Schedules · Usage**, the active session title, the enforcement badge, and the settings gear. Agents, Schedules, and Usage are pages; session history is *not* a sidebar panel — the Agents page (renamed from "Sessions" 2026-09-19, operator direction: a more truthful description that avoids confusion with transcript history) and the `/agents` composer command (with `/chat`, `/schedules`, `/usage`; `/sessions` kept as an alias) are its homes. Below 1100px the slugs keep their icons and drop the names.
- **Parallel sessions, one focused at a time.** Multiple live ACP runtimes run side by side (server-side cap with LRU eviction); the chat view focuses one session, and every session-scoped fetch/poll/mutation carries that session's id (`?session=`). New sessions pick their agent at creation; the Agents page shows live facts per card (run dot = turn in flight, focused tag, agent, relative time) with focus/open-chat, agent switch, rename, dismiss, and clear-unused. Parked permission prompts are per session — switching focus never cancels another session's prompt.
- **Schedules page (W074) and loop projection (W073).** The Schedules nav slug projects the hub cron table live (pause/resume, delete, next-run) and the self-improvement loop registry (state, iterations, cancel); both ride the web service's hub proxy (`/api/schedules*`, `/api/loops*`) so the hub stays the single writer and the browser never holds a hub token. The verifier-credential actions — loop start (`workflow-rsi start`) and schedule run-now — are deliberately CLI-only, and the page says so instead of hiding it. Rename discipline: only user-facing language (nav slug, heading, aria labels, composer command) plus the component/file name changed; the `/api/sessions*` endpoints and `SessionMeta` keep their wire names because tests pin them, recorded here rather than silently.
