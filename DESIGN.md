...
- **Testing is part of the surface.** `npm run test:webui` drives real Chromium over raw CDP (no Playwright dependency) against a scripted fake agent: boot, one streamed turn, palette switch, console/exception cleanliness; SSR pins (`test/webapp-surface.test.ts`) hold markup and square-edge invariants; `test/webapp-palettes.test.ts` gates every palette × mode on WCAG AA; `test/openrouter-analytics.test.ts` pins the usage data path.
